...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("Detected userId:", userId, "Type:", typeof userId);

    // --- GET / ---
    if (req.method === "GET" && !url.searchParams.has('estimate') && !url.pathname.includes('/api-tokens')) {
    ...
    // --- API TOKENS ---
    if (url.pathname.endsWith('/api-tokens') || url.pathname.includes('/api-tokens/')) {
      if (req.method === "GET") {
        console.log("Fetching tokens for userId:", userId);
        const { data, error } = await supabase.from("auth_tokens").select("*").eq("user_id", userId).order("created_at", { ascending: false });
        if (error) {
           console.error("Error fetching tokens:", error);
           throw error;
        }
        console.log("Tokens found:", data?.length || 0);
        return jsonResponse({ success: true, result: data?.map((t: any) => ({
          token: t.token,
          userId: t.user_id,
          name: t.name,
      ...
      if (req.method === "DELETE") {
        ...
        const { error } = await supabase.from("auth_tokens").delete().eq("user_id", userId).eq("token", tokenToDelete);
        if (error) throw error;
