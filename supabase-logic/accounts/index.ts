/**
 * Accounts (Identities) Edge Function - v21
 * Fix: Thêm updated_at/created_at vào credential upsert + error handling
 */
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if ((path === "/connect" || path === "/connect/") && method === "POST") {
            ...
            // FIX: Include created_at and updated_at to avoid NOT NULL constraint violation
            ...
            if (credError) {
                console.error(`[Connect] Credential upsert error:`, credError);
                return jsonResponse({ success: false, error: `Lưu token thất bại: ${credError.message}` }, 500);
            }
        ...
        if (path.includes("/sync-accounts")) {
            ...
            if (!token) return jsonResponse({ success: false, error: "Không tìm thấy Access Token hợp lệ trong DB. Vui lòng thêm token trước.", logs }, 400);
            ...
            for (const acc of (fbAccs.data || [])) {
                ...
                const { data, error: upsertError } = await supabase.from("platform_accounts").upsert(upsertData, { onConflict: "platform_identity_id,platform_id,external_id" }).select().single();
        ...
        if (path.includes("/update-token") && method === "POST") {
            ...
            // FIX: Include created_at and updated_at to handle both INSERT and UPDATE cases
            const now = new Date().toISOString();
            const { error: credError } = await supabase.from("platform_credentials").upsert({
                platform_identity_id: identity.id,
                credential_type: "access_token",
            ...
            if (credError) {
                console.error(`[UpdateToken] Credential upsert error:`, credError);
                return jsonResponse({ success: false, error: `Lưu token thất bại: ${credError.message}` }, 500);
            }

            // Mark identity as valid again since we have a new token
            await supabase.from("platform_identities").update({ is_valid: true, updated_at: now }).eq("id", identity.id);
