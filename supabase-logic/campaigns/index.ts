...
async function verifyAuth(req: Request) {
  ...
  // FALLBACK 2: Supabase Auth (Native)
  if (authHeader?.startsWith("Bearer ")) {
    const token = authHeader.substring(7).trim();
    try {
  ...
  // PERMISSIVE FALLBACK
  console.log("[Auth] Permissive fallback triggered (no valid token)");
  return { userId: 1 };
}
  ...
  try {
    ...
    if (path.includes("/by-account/") && req.method === "GET") {
      ...
      const { data, error } = await supabase
        .from("unified_campaigns")
        .select(`
          id, name, status, effective_status, external_id, platform_account_id,
          platform_accounts!inner(platform_identities!inner(user_id))
        `)
    ...
    if (path.includes("/sync/account/") && req.method === "POST") {
      ...
      // 1. Verify account ownership (Superuser bypass)
      let query = supabase
      ...
      // 2. Forward with service key to ensure internal sync worker is authorized
      console.log(`[Campaigns] Triggering sync for account ${accountId}`);
      ...
      try {
        ...
        console.error(`[Campaigns] Sync response not JSON: ${responseText.slice(0, 500)}`);
        return jsonResponse({
          success: false,
    ...
    if (subPathSegments.length === 1 && req.method === "GET") {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          id, external_id, name, status, effective_status, daily_budget, start_time, end_time, synced_at, platform_account_id,
          platform_accounts!inner(id, name, platforms(code), platform_identities!inner(user_id))
        `)
        .eq("id", campaignId)
        .eq("platform_accounts.platform_identities.user_id", auth.userId)
        .maybeSingle();
